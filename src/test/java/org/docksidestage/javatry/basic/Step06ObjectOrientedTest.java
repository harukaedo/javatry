...
public class Step06ObjectOrientedTest extends PlainTestCase {
    ...
    public void test_objectOriented_aboutObject_usingObject() {
        ...
        Ticket ticket = booth.buyOneDayPassport(10000).getTicket(); // also here
    ...
    // done jflute 今度1on1にオブジェクトとは？ (2025/11/14)
    ...
    public void test_objectOriented_polymorphism_interface_whatisdifference() {
        ...
        // #1on1: ↑これはこれで良いポイントをついている。結果的にそうなる。 (2026/01/29)
        // まず、コンセプトが違う。
        // オブジェクト指向 :: "もの" そのものにフォーカスを当てて分析して整理整頓する考え方
        // インターフェース :: 操作にフォーカスを当てて分析して整理整頓する考え方 (実体が何かは気にしない)
        //
        // 違うコンセプトのものが、一つのJavaというフィールドに混在してきている。
        // コンセプトが違うので、Javaでの表現方法も違う。
        // 互いにMTGしてるわけではないので、似たところも出てくる。
        // 
        // オブジェクト指向 :: ポリモーフィズム、実装継承(処理の再利用)
        // インターフェース :: ポリモーフィズム(のみ)
        //
        // 機能だけ見ると、包含してるように見えて、インターフェースは必要なのか？になる。
        // Swimmable は、抽象クラスでも良かったのか？
        // Javaは、多重継承ができないようになっている。
        // 昔C++で、多重継承できてたけど、なかなかカオスになりやすかったので...Javaではやめた。
        // Javaは、単一継承しかできないようにした。
        // でも、それは人間の都合であって物事の道理は、やはり多重継承な場面が出てくる。
        //
        // そこでインターフェース。Javaのinterfaceは、多重implementsできる。
        // でもそれって結局、カオスな多重継承ができちゃうのでは？
        // interfaceは、(基本的に)実装継承はできない。
        // カオスが発生するのは、多重ポリモーフィズムではなく、多重実装継承の方なので、
        // interfaceは都合よく、なんとか回避。
        //
        // 「単一継承のオブジェクト指向 + ポリモーフィズムのみのインターフェース」でバランス。
        // (実装継承だけが再利用の手段というわけでもないので、多重実装継承は捨てる)
        //
        // TypeScriptのインターフェースのお話:
        // https://typescriptbook.jp/reference/object-oriented/interface
        
        // TODO jflute 次回1on1, インターフェースの応用編 (2026/01/29)
    }
     ...
     */
    //0319メモ：コンクートクラスの対象となるのがmac/windows/oldwindowsになるのでそこで切り分けたい
    ...
    public void test_objectOriented_writing_specialization_extractToConcrete() {
        ...
        St6OperationSystem windows = new St6Windows("chikuwa");
        St6OperationSystem oldWindows = new St6OldWindows("chikuwa");

        log(mac.buildUserResourcePath("chikuwa.jpeg"));
        log(windows.buildUserResourcePath("chikuwa.jpeg"));
        log(oldWindows.buildUserResourcePath("chikuwa.jpeg"));
    }
