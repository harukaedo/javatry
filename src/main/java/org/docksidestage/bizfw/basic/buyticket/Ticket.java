...
//private int restDays; // チケットの残り使用可能日数
    ...
    //
    // done edo [読み物課題] リファクタリングは思考のツール by jflute (2026/02/13)
    // https://jflute.hatenadiary.jp/entry/20121202/1354442627
    // 最終的な形を作ってからじゃないと、良い名前というのはなかなか思い浮かばないもの
    //0220感想メモ：実際に、配属されてから業務を与えられた時は完璧なコードを書かなければとプレッシャーを
    //感じてしまう場面もあったが、最初から綺麗で完璧なものを求めてしまうのは時間がもったいないということ
    //がわかった。質よりもスピードを大事にしコツコツ組み立て行くことが大切であるというと学んだ。
    //リファクタはコードが動いた後にする別作業ではなく、コードを書きながらセットで行うプログラミングの一部である
    //という発想を持ち、リファクタリングをするということにも前向きに取り組んでいきたい。
    //
    // done edo [読み物課題] リファクタリングという行為が好きか？ by jflute (2026/02/13)
    // https://jflute.hatenadiary.jp/entry/20220328/loverefactor
    //0220感想メモ:実際にリファクタの時間を設けていないといつかやろうのリファクタは確かに後回しにされてしまい、
    //結果として、コードがどんどん複雑になってしまうということがわかった。
    //リファクタリングはコードが動いた後にする別作業ではなく、コードを書きながらセットで行うプログラミングの一部である
    //という発想を持ち、リファクタリングをするということにも前向きに取り組んでいきたい。
    //コード全体を俯瞰して見る事でそのリポジトリの設計思想などを理解できることも多いためどんどんチャレンジしていきたい。
    // #1on1: コードをいかにして俯瞰するか？が全体設計をよくするキーワード (2026/02/26)
